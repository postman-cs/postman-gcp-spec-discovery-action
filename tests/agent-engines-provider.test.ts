...
describe('Vertex AI Agent Engine provider', () => {
  it('assembles OpenAPI from classMethods but remains local-derived and unsupported for auto-export', async () => {
    ...
    expect(candidates[0]?.evidence.join(' ')).toContain('local-derived');
  ...
  it('marks invalid generated declarations local-derived unsupported for manual review', async () => {
    const candidates = await new AgentEnginesProvider(client([{ description: 'missing name' }]), { projectId: 'p' }).listCandidates();
    expect(candidates[0]).toMatchObject({ authority: 'local-derived', supported: false });
  ...
  it('marks unsupported api_modes local-derived unsupported for manual review', async () => {
    const candidates = await new AgentEnginesProvider(client([
