 ...
 * v2.10.26 cut in postman-bootstrap-action:
 *
...
function executeRelease(plan) {
  ...
  run('npm', ['run', 'lint']);
  run('npm', ['test']);
  run('node', ['scripts/render-action-tables.mjs', '--check']);
  ...
  if (sourceCommit === releaseCommit) {
    throw new Error('release commit unexpectedly equals source commit');
  }
