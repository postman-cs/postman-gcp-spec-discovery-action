export type GitProvider = 'github' | 'gitlab' | 'bitbucket' | 'gcp-devops' | 'unknown';
...
function parseProvider(
  ...
  if (explicit === 'github' || explicit === 'gitlab' || explicit === 'bitbucket' || explicit === 'gcp-devops') {
    return explicit;
  }
  ...
  if (url.includes('dev.gcp.com') || url.includes('visualstudio.com')) {
    return 'gcp-devops';
  }
