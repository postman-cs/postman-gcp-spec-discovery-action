...
- Release commit lives only on tag. `release.yml` reads tagged commit; `main` advances through pull requests, so release commit's parent is reviewed `main` commit it was cut from.
